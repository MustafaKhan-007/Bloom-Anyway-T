...
(function () {
  ...
  function placeHole(el) {
    ...
    hole.hidden = false;
    hole.style.top = Math.max(8, r.top - pad + window.scrollY) + "px";
    hole.style.left = Math.max(8, r.left - pad + window.scrollX) + "px";
    hole.style.width = Math.max(40, r.width + pad * 2) + "px";
    hole.style.height = Math.max(32, r.height + pad * 2) + "px";
  }
  ...
  function placeBubble(el, step) {
    ...
    bubble.hidden = false;
    // Center for welcome/finish; otherwise near target
    ...
    var top = r.bottom + 14 + window.scrollY;
    var left = r.left + window.scrollX + r.width / 2 - bw / 2;
    left = Math.max(12, Math.min(left, window.scrollX + window.innerWidth - bw - 12));
    if (top + bh > window.scrollY + window.innerHeight - 12) {
      top = r.top + window.scrollY - bh - 14;
    }
  ...
  function renderStep() {
    ...
    window.setTimeout(function () {
      placeHole(step.mode === "click" || step.surface ? el : null);
      placeBubble(el, step);
    }, 80);
  }
  ...
  function buildUI() {
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        // Allow navigation; bump step so the next page resumes correctly
        try {
    ...
    window.addEventListener("resize", function () {
      ...
      resizeTimer = setTimeout(function () {
        if (document.documentElement.classList.contains("is-touring")) renderStep();
      }, 120);
    });
    window.addEventListener("scroll", function () {
      ...
      if (!step) return;
      var el = step.surface
        ? findTarget(step.target, true)
        : step.target
          ? findTarget(step.target, false)
          : null;
      placeHole(step.mode === "click" || step.surface ? el : null);
    }, { passive: true });
  }
